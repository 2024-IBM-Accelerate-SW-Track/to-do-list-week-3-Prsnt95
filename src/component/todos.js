...
const Todos = ({ todos, deleteTodo }) => {
  const todoList = todos.length ? (
    todos.map((todo) => {
      const overdue = new Date(todo.due) < new Date();
      const dueColor = overdue ? "#ffcccc" : "#ffffff"; // Red for overdue, white for not overdue

      return (
        <Grid key={todo.id}>
          <Card style={{ marginTop: 10, background: dueColor }}>
